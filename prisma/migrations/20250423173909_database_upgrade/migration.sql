...
CREATE TABLE `Schedule` (
    ...
    UNIQUE INDEX `Schedule_workplaceId_positionId_key`(`workplaceId`, `positionId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `ScheduleChange` (
    ...
    `newCheckIn` TIME(6) NOT NULL,
    `newCheckOut` TIME(6) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `ReportAttendance` ADD CONSTRAINT `ReportAttendance_scheduleId_fkey` FOREIGN KEY (`scheduleId`) REFERENCES `Schedule`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
